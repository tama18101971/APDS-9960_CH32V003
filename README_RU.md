...
- Минимальное использование RAM (~32 байт статики драйвера, пиковый стек ~148 байт внутри `apds_readGesture()`)
- Минимальный размер Flash (~2.4 КБ только драйвер, ~11.9 КБ полный демо-проект)
...
- Реальный таймаут чтения жеста (свободнобегущий SysTick под управлением драйвера, либо бюджет итераций при `APDS_OWN_SYSTICK=0`)
...
#define APDS_ERR_SENSOR_HANG    3   // Датчик не отвечает (также при провале sensor_reinit() внутри восстановления после GFOV)
#define APDS_ERR_INVALID_ID     4   // Устройство по адресу 0x39 не APDS9960
#define APDS_ERR_UNSUPPORTED    5   // Функция выключена на этапе компиляции (apds_recalibrate() при APDS_ENABLE_CALIBRATION=0)
```
...
int main(void) {
    ...
    while (1) {
        ...
        // Примечание: apds_clearInterrupt() здесь опционален — apds_available()
        // читает GSTATUS сам, а GINT в сенсоре сбрасывается опустошением FIFO
        // внутри apds_readGesture().
        ...
        if (apds_available()) {
            ...
            // Дренаж хвоста FIFO. Ограничен: аппаратный FIFO — 32 пакета,
            // поэтому достаточно не более 8 проходов.
            uint8_t drain = 8;
...
Аппаратный FIFO (4 байта на пакет: [U, D, L, R])
        |
        v
Драйвер считывает FIFO через I2C (порциями до 8 пакетов за транзакцию)
        |
        v
Вычисление соотношений: (U-D)*100/(U+D), (L-R)*100/(L+R)
        |
        v
Разность усреднённых краёв: avg(последние N) - avg(первые N), N=2
        |
        v
Определение направления по дельтам краёв
```
...
### Разность усреднённых краёв

Направление жеста — это разность между **усреднёнными краями**
последовательности валидных пакетов:

```
ud_delta = avg(UD_ratio последних N пакетов) - avg(UD_ratio первых N пакетов)
lr_delta = avg(LR_ratio последних N пакетов) - avg(LR_ratio первых N пакетов)
```

где N = 2. Математически это эквивалентно прежней «сумме приращений ratio»
(та сумма телескопируется в `последний - первый`), но усреднение краёв
устойчиво к одиночному шумному кадру на границе свайпа, который раньше
мог целиком определить результат. Подход работает и для быстрых, и для
медленных жестов: свайп даёт монотонный сдвиг ratio между началом и концом.
...
2. Сравниваются абсолютные дельты краёв: `|ud_delta|` vs `|lr_delta|`
3. Доминирующая ось определяет жест:
   - `|ud_delta| > |lr_delta|` → вертикальный жест (UP или DOWN)
   - `|lr_delta| > |ud_delta|` → горизонтальный жест (LEFT или RIGHT)
...
`PIHT`/`PILT`/`PERS` драйвером не пишутся: proximity-прерывания (`PIEN`)
никогда не включаются, поэтому эти регистры ни на что не влияли бы.
`APDS_LED_BOOST` (CONFIG2, по умолчанию 300%) масштабирует фактический ток
LED и является главной причиной насыщения PDATA при близкой руке.
...
`APDS_GESTURE_TIMEOUT_MS` ограничивает один вызов `apds_readGesture()`
(1..1000 мс). При дефолтном `APDS_OWN_SYSTICK=1` драйвер на время вызова
переконфигурирует SysTick как свободнобегущий счётчик — не используйте
этот режим, если приложение ведёт собственный таймбейс на SysTick
(тогда установите 0: цикл ограничивается бюджетом итераций, каждая из
которых занимает не менее 1 мс).

Направление жеста вычисляется как разность **усреднённых краёв**:
`delta = avg(последние N валидных пакетов) − avg(первые N валидных пакетов)`
по каждой оси (N = 2). Это математически эквивалентно прежней
телескопической сумме приращений ratio, но одиночный шумный кадр на границе
свайпа больше не определяет жест целиком. Изменение метрики означает, что
значения `APDS_GESTURE_SENSITIVITY`, подобранные для старых версий, могут
потребовать перенастройки (рабочий диапазон обычно 5..40).
...
| Несколько жестов на один свайп | `apds_readGesture()` ждёт фактического сброса GVALID, ограниченного `APDS_GESTURE_TIMEOUT_MS`. Если всё ещё происходит — проверьте целостность сигнала I2C на 400 кГц. |
...
|--------|----------------|--------------------|-------|
| RAM | ~32 байт статики | ~512 байт | 2048 байт |
| Flash | ~2.4 КБ | ~11.9 КБ | 16384 байт |
...
`printf`. Пиковая глубина стека внутри `apds_readGesture()` (драйвер + цепочка
I2C) — ~148 байт против 256 байт, резервируемых штатным линкер-скриптом.
...
- `GINT` сбрасывается полным опустошением FIFO, а не чтением `GSTATUS`; отдельной команды сброса gesture-прерывания нет
- При `APDS_OWN_SYSTICK=1` (по умолчанию) SysTick переконфигурируется драйвером во время `apds_readGesture()`; приложениям с собственным таймбейсом на SysTick следует установить 0
