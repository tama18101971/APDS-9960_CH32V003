...
Этот документ описывает, как использовать драйвер жестов APDS9960 (файлы `i2c.*`,
...
|------|-------------|------------|
| `i2c.h` / `i2c.c` | ✅ Всегда | Драйвер I2C1 для CH32V003 (общего назначения) |
...
**Важно:** `i2c.c`/`i2c.h` — самостоятельный переиспользуемый I2C-драйвер
(комментарий в файле явно указывает: "Применение: APDS-9960, DAC7571, EEPROM,
OLED и др."). Если в целевом проекте уже есть свой I2C-драйвер для CH32V003,
не подключайте `i2c.c` повторно — сверьте API (`i2c_init`, `i2c_read_register`,
`i2c_write_register`, `i2c_read_buffer`, `i2c_write_buffer`) и переиспользуйте то,
что уже есть.
...
3. В своём `src/main.c` подключайте как обычно:
...
4. `pio run` — PlatformIO подхватит `lib/APDS9960/` автоматически, без правок
   `platformio.ini` (кроме платформы/фреймворка из раздела 2).
...
- [ ] Если в проекте уже есть свой I2C-драйвер — сверить API, не подключать `i2c.c` повторно
