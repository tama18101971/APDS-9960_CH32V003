...
- Свободные пины: **PC1 (SDA)**, **PC2 (SCL)**, и (только для interrupt-режима)
  **PC3 (INT)**. Если эти пины заняты другой периферией в целевом проекте —
  ...
  `APDS_INT_PIN_SOURCE`) через `build_flags` (см. раздел 9).
...
int main(void) {
    ...
    uint8_t i2c_st = i2c_init(400000);   /* Fast-mode I2C, датчик и I2C-CH32V003 это поддерживают */
    if (i2c_st != I2C_OK) {              /* отсекается до любых обращений к датчику */
...
## 8. Подключение пинов (не меняется независимо от способа интеграции)
...
## 9. Настройка под конкретный проект
...
## 10. Бюджет ресурсов (учитывайте в новом проекте)

На чипе CH32V003 (RAM 2 КБ, Flash 16 КБ) сам драйвер занимает примерно:

- **RAM:** ~21 байт статики + до ~150 байт стека на пике (пакетное чтение FIFO)
- **Flash:** ~3.3 КБ (`.text` объектов `apds9960.o` + `int_config.o`, без I2C-библиотеки)
...
## 11. Чек-лист совместимости перед переносом
...
- [ ] Хватает Flash/RAM (см. раздел 10)
- [ ] В `platformio.ini` добавлена `lib_deps` с `I2C-CH32V003`

## 12. Типичные проблемы при переносе
